    ...
    def run(self):
        while self.game_started:
            ...
            if self.menu_active:
                ...
                if self.time <= 0:
                    game_over = GameOver(self.player.score, int(self.player.radius))
    ...
    def run(self):
        while self.game_started:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        if not self.menu_active:
                            ...
                            self.time = Params.CHRONO_START_TIME
                            self.player.score = 0
                            self.player.radius = Params.PLAYER_RADIUS
                            self.menu.selected_difficulty = "EASY"
                            self.menu.difficulty_checked = False
                            self.food_list = []
                            self.trap_list = []
                            self.player.reset_position(Params.SCREEN_WIDTH // 2, Params.SCREEN_HEIGHT // 2)
            ...
            if self.menu_active:
                ...
                if self.time <= 0:
                    game_over = GameOver(self.player.score, int(self.player.radius))
                    result = game_over.display_game_over()
                    if result == "PlayAgain":
                        self.menu_active = True
                        self.game_started = True
                        self.time = Params.CHRONO_START_TIME
